...
export function TrackAnalysis(props) {
    ...
    return (<div>
        ...
        <p><strong>Danceability: </strong>describes how suitable a track is for dancing based on tempo, rhythm stability, beat strength, and overall regularity (0 is least danceable, 1 is most)</p>
        <p><strong>Energy: </strong>represents the perceptual measure of intensity and activity based on dynamic range, perceived loudness, timbre, onset rate, and general entropy (1 is fast, loud, and noisy)</p>
        <p><strong>Valence: </strong>describes the musical positiveness conveyed (0 is sad, depressed, or angry while 1 is happy, cheerful, or euphoric)</p>
        <p><strong>Instrumentalness: </strong>predicts whether a track contains no vocals ("ooh" and "aah" sounds are treated as instrumental). The closer the Instrumentalness is to 1, the greater likelihood the track contains no vocal content</p>
        <p><strong>Speechiness: </strong>detects the presence of spoken words in a track (values below 0.33 most likely represent music, values close to 1 represent spoken-word)</p>
    </div>);
}
